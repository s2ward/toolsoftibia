...
//const FETCH_BOOKS = fetch('https://resources.talesoftibia.com/data/books.json')
//    .then(response => response.json())
//    .catch(handleFetchError);
...
//const FETCH_IMG = fetch('https://resources.talesoftibia.com/data/books_img.json')
//    .then(response => response.json())
//    .catch(handleFetchError);

Promise.all([FETCH_BOOKS])
    .then(([books]) => {
        booksData = books;
...
function setUpFormListeners() {
    ...
    searchPhraseInput.addEventListener('input', () => {
        clearTimeout(timer);
        timer = setTimeout(handleInputDebounced, 200);
    });
...
function searchSpecificObject(keyword) {
    ...
    for (const obj of booksData) {
      console.log(`Checking book: ${obj.name}`);
        ...
        // For some ungodly reason, this console log is needed to accurately find multiple books, e.g. "Donina" returns Formula 1 and Formula 2 if this console log is present, otherwise it returns the book that is first in order in books.json
        const originalConsoleLog = console.log;
        console.log = function() {};
        console.log(`Book: ${obj.name}, Match: ${KEYWORD_REGEX.test(obj.author.toLowerCase())}`);
    ...
    if (searchModeValue === 1) {
        BOOK_TEXTS_ARRAY_DIV_CONTENT = '<div class="multiColumnContent">' + [...BOOK_TEXTS_ARRAY].map(obj => {
            // Updated section for wrapping text in <p> tags and highlighting keywords
            const wrappedText = obj.text.toString().split('\n').map(line => {
                if (line.trim()) {
                    // Apply the keyword highlighting within each paragraph
                    const highlightedLine = line.trim().replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/"/g, '&quot;').replace(/'/g, '&#39;')
                        .replace(KEYWORDS_REGEX, '<a>$1</a>');  // Use the keyword regex to wrap matches in <a> tags
                    return `<p>${highlightedLine}</p>`;
                }
                return '';  // Return an empty string for lines that are only whitespace
            }).join('');
            ...
            return `
                <div class="bookBoxContent tibiaStyleBorderHigh2 bottomSpace10M">
                    <div class="flexRow bookHeader" onClick="showSpecificObject(\`${obj.name}\`, false); showInformation(\`${obj.name}\`);">
                        <div style="background-image: url('https://resources.talesoftibia.com/images/${obj.type}s/${obj.img[0].replace(/ /g, '_').replace(/'/g, "\\'")}.png')"
                            class="bookImage">
                        </div>
                        <div class="bookName fullWidth rightSpace10P tibiaStyleTextLight">${obj.name.replace(/_/g, ' ')}</div>
                    </div>
    
                    <div class="bookText tibiaStyleBorderDeep1 spaceAround5P leftSpace10M rightSpace10M bottomSpace10M tibiaStyleTextLight">${wrappedText}</div>
                </div>
            `;
        }).join('') + '</div>';
    } else {
        BOOK_TEXTS_ARRAY_DIV_CONTENT = '<div class="allNpcBox">' + [...BOOK_TEXTS_ARRAY].map(obj => `
            <div class="npcBox" onClick="showSpecificObject(\`${obj.name}\`, false); showInformation(\`${obj.name}\`);">
                ...
                <div class="npcAppearance" style="background-image: url('https://resources.talesoftibia.com/images/${obj.type}s/${obj.img[0].replace(/ /g, '_').replace(/'/g, "\\'")}.png')">
                </div>
            </div>
        `).join('') + '</div>';
    }
  ...
  const OBJ_DIV_CONTENT = `
      <div style="width: 29.75rem; margin: auto; display: block;" class="bookBoxContent tibiaStyleBorderHigh2">
          <div class="flexRow bookHeader" onClick="showInformation(\`${FOUND_OBJ.name}\`);">
              <div
                  class="bookImage"
                  style="background-image: url('https://resources.talesoftibia.com/images/${FOUND_OBJ.type}s/${FOUND_OBJ.img[0].replace(/ /g, '_').replace(/'/g, "\\'")}.png')">
              </div>
...
function showInformation(objName) {
    ...
    if (FOUND_OBJ) {
        const INFO_DIV_CONTENT = `
            ...
            <div class="npcMonologues topSpace5M">
            ...
            <div class="flexRow">${[...FOUND_OBJ.img].map(img => `
                ...
                style="background-image: url('https://resources.talesoftibia.com/images/${FOUND_OBJ.type}s/${img.replace(/ /g, '_').replace(/'/g, "\\'")}.png')"></div>
            `).join('')}
            </div>
                ...
                <table class="tibiaStyleTable userSelectLock">
                    <tbody class="tibiaStyleTbody">
                        ...
                        <tr class="tibiaStyleTr">
                            ...
                            <td class="tibiaStyleTd">${FOUND_OBJ.locations.join('<p></p>')}</td>
                        </tr>
                    </tbody>
                </table>
            </div>
...
document.addEventListener('DOMContentLoaded', function() {
    document.querySelectorAll('.bookText').forEach((bookText) => {
      ...
      bookText.innerHTML = bookText.innerHTML.split('<br>').map(line => {
        return line.trim() ? `<p>${line.trim()}</p>` : ''; // Wrap non-empty lines in paragraph tags
      }).join('');
    });
  });